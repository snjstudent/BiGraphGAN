...
class KeyDataset(BaseDataset):
    ...
    def init_categories(self, pairLst):
        ...
        for i in range(self.size):
            pair = [pairs_file_train.iloc[i]['from'],
                    pairs_file_train.iloc[i]['to']]
    ...
    def __getitem__(self, index):
        ...
        BP1_path = os.path.join(self.dir_K, P1_name +
                                '.npy')  # bone of person 1

        # person 2 and its bone
        P2_path = os.path.join(self.dir_P, P2_name)  # person 2
        BP2_path = os.path.join(self.dir_K, P2_name +
                                '.npy')  # bone of person 2

        P1_img = Image.fromarray(imageio.imread(
            P1_path)).resize((176, 256)).convert('RGB')
        P2_img = Image.fromarray(imageio.imread(
            P2_path)).resize((176, 256)).convert('RGB')
        print(P1_img.size)
        print(P2_img.size)
